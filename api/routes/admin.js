...
async function verifyDoctor(req,res)
{
    try
    {
        ...
        if(!doctor)
        {
            return res.status(400).send({success:false,msg:"No doctor application found"})
        }
        await User.updateOne({_id:doctor.userId},{
            $set:{
...
async function verifyMerchant(req,res)
{
    try
    {
        ...
        if(!merchant)
        {
            return res.status(400).send({success:false,msg:"No Merchant application found"})
        }
        await User.updateOne({_id:merchant.userId},{
            $set:{
                'isMerchant.isVerified':true,
                'isMerchant.merchantId':merchantId
            }
        })
        return res.status(200).send({success:true,msg:"Merchant Verified"})
    }
    catch(err)
    {
        console.log(err)
